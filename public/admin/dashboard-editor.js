...
let ED = {
  ...
  cropRect: null,
  isDraggingImage: false, isDraggingCrop: false, isResizingCrop: false,
...
async function openEditor(id) {
  ...
  try {
    ...
    img.onload = () => {
      ...
      if (!vig) { vig = document.createElement('div'); vig.className = 'vignette-overlay'; vig.style.cssText = 'position:absolute;inset:0;pointer-events:none;z-index:4;'; $('editPreview').appendChild(vig); }
      vig.style.opacity = 0;
    };
...
function fitImageToPreview() {
  if (!ED.img) return;
  const preview = $('editPreview'); if (!preview) return;
  ...
  const zs = $('zoomSlider'); if (zs) { zs.min = Math.min(fit, 0.1).toFixed(2); zs.max = 4; zs.value = fit.toFixed(2); }
...
function renderCropRect() {
  ...
  const r = ED.cropRect, rect = $('cropRect'), W = $('editPreview').clientWidth, H = $('editPreview').clientHeight;
...
(function initCropSystem() {
  const preview = $('editPreview'); if (!preview || preview.dataset.cropReady) return;
