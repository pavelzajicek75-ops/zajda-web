export async function onRequestGet(context) {
  ...
  for (const obj of list.objects || []) {
    try {
      ...
      if (!data) continue;
      let item, raw = null;
      try { item = await data.json(); } catch { raw = await data.text(); item = { text: raw }; }
      ...
      if (Array.isArray(item)) { for (const sub of item) { const st = sub.text || sub.quote || sub.content || sub.citat || ''; const sa = sub.author || sub.autor || sub.by || sub.source || ''; if (st) quotes.push({ key: obj.key, text: String(st), author: String(sa), created: sub.created || obj.uploaded }); } continue; }
