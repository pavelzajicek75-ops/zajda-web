// gallery rebuild v10 – UTF‑8 FIX + DELETE FIX
...
async function deletePhoto(name) {
  const res = await fetch(`/api/photo/${encodeURIComponent(name)}/delete`, {
    method: "DELETE"
  });
...
async function deleteSelected() {
  for (const name of selected) {
    await fetch(`/api/photo/${encodeURIComponent(name)}/delete`, {
      method: "DELETE"
    });
  }
  await loadGallery();
